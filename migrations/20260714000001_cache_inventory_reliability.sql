...
CREATE TRIGGER IF NOT EXISTS james_cache_inventory_insert
AFTER INSERT ON james_cache_artifacts
BEGIN
    UPDATE cache_inventory_state SET generation = generation + 1 WHERE singleton = 1;
END;

CREATE TRIGGER IF NOT EXISTS james_cache_inventory_update
AFTER UPDATE ON james_cache_artifacts
BEGIN
    UPDATE cache_inventory_state SET generation = generation + 1 WHERE singleton = 1;
END;

CREATE TRIGGER IF NOT EXISTS james_cache_inventory_delete
AFTER DELETE ON james_cache_artifacts
BEGIN
    UPDATE cache_inventory_state SET generation = generation + 1 WHERE singleton = 1;
END;
...
ALTER TABLE james_cache_artifacts ADD COLUMN verification_status TEXT NOT NULL DEFAULT 'pending';
ALTER TABLE james_cache_artifacts ADD COLUMN last_verified_at TEXT;
...
CREATE INDEX IF NOT EXISTS idx_james_cache_artifacts_verification
    ON james_cache_artifacts(last_verified_at, created_at, id);
