...
pub(crate) fn validate_plan_delivery(
    ...
    match (plan.schema.as_str(), layout) {
        ...
        (INSTALL_PLAN_SCHEMA_V1, MediaLayout::Thin) => {
            bail!("thin Forge media requires a signed network snapshot install plan")
        }
        (INSTALL_PLAN_SCHEMA_V2, MediaLayout::Embedded) => {
            bail!("embedded Forge media cannot use network snapshot delivery")
        }
        _ => bail!("install plan package delivery does not match this Forge media"),
    }
}
...
fn verified_marker(release: &SignedApplianceRelease) -> VerifiedSnapshotMarker {
    VerifiedSnapshotMarker {
        schema: "cybex.forge.verified-appliance-snapshot.v1".to_string(),
...
mod tests {
    ...
    fn release(canonical_url: &str) -> SignedApplianceRelease {
        SignedApplianceRelease {
            schema: "cybex.forge.appliance-release.v1".into(),
    ...
    fn production_transport_must_equal_the_release_signed_https_url() {
        let url =
            "https://releases.cybex.net/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst";
        assert!(validate_transport_url(url, &release(url)).is_ok());
        assert!(
            validate_transport_url(
                "https://mirror.example/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
                &release(url)
            )
            .is_err()
        );
    ...
    fn qualification_transport_is_an_explicit_private_ip_endpoint() {
        ...
        assert!(
            validate_transport_url(
                "http://192.168.122.1:8080/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
                &release
            )
            .is_ok()
        );
        assert!(
            validate_transport_url(
                "http://[fd00::1]:8080/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
                &release
            )
            .is_ok()
        );
        for invalid in [
            "http://example.test:8080/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
            "http://203.0.113.2:8080/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
            "http://127.0.0.1/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
            "http://127.0.0.1:8080/wrong.tar.zst",
            "http://user@127.0.0.1:8080/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst",
            "http://127.0.0.1:8080/cybex-forge-appliance-packages-0.1.2-x86_64-linux.tar.zst?token=x",
        ] {
            assert!(
